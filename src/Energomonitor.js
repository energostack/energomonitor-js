...
const ENDPOINT_URLS = {
    ...
    streamData: (feedId, streamId) =>
        `/feeds/${feedId}/streams/${streamId}/data`,
...
class Energomonitor {
    ...
    constructor(
        token,
        axiosInstance,
        apiURL = 'https://api.energomonitor.com/v1'
    ) {
        this._token = token;
    ...
    authorize(username, password, note, resources, validMinutes) {
        ...
        return this._axios.post(url, data, config).then(response => {
            ...
            this._token = token;
            this._authorizedRequestHeaders = buildAuthorizedRequestHeaders(
                token
            );
